...
- `trainer.command`, `trainer.code_root`, and `trainer.arguments`: the standalone trainer invocation
  ...
  Importable native Python extensions and every resolved transitive ELF dependency are copied into
  individually sealed memory files. The exact loader names, resolved paths, dependency bytes,
  interpreter loader, and effective library-search inputs are hashed into the environment identity.
  ...
  environment paths are removed from the child import search path. Generated compilation is accepted
  only when a sealed source caller registers the exact source payload before its resulting code can
  execute; execution also records the complete marshalled code object, including constants and
  nested code, so a bound filename or bytecode string cannot hide different generated semantics.
  Content-addressed
  generated-code records are retained in an attempt-level execution-recipe hash inside the
  authority-signed journal, so append-safe reuse binds the actual generated bytes rather than
  trusting a caller filename. The seal is bounded to
...
- `elapsed_time_anchors`: required for every training seed before a run. Each training seed
  ...
  was issued earlier. Immediately before an active seed begins, the authority durably opens a
  seed-local timing segment using both wall and monotonic clocks. Every journal seal consumes exactly
  one such segment and enforces its independently observed duration in addition to the caller's
  conservative floor, so caller under-reporting cannot shorten active work without charging idle
  anchor age or another seed's execution.
...
low-level `start-timing-segment` operation accepts the registered seed and `started_unix_ns`; the
public evidence command invokes it itself, so operators do not need to add it to manifests. Copying
